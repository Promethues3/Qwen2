...
def parse_messages(messages, functions):
    ...
    if functions:
        ...
        for func_info in functions:
            name_m = func_info['function']['name']
            name_h = func_info['function']['name']
            desc_m = func_info['function']['description']
            tool = TOOL_DESC.format(
                ...
                parameters=json.dumps(
                    func_info['function']["parameters"], ensure_ascii=False),
            )
    ...
    for m_idx, m in enumerate(_messages):
        ...
        if role == "function":
            ...
            last_msg_has_zh = len(re.findall(
                r"[\u4e00-\u9fff]+", last_msg)) > 0
    ...
    print(messages)
    # messages0 = [ChatMessage(role='system', content=system)]
...
async def create_chat_completion(request: ChatCompletionRequest):
    ...
    global device_map
    model_inputs = tokenizer([text], return_tensors="pt").to(device_map)
    ...
    if query is _TEXT_COMPLETION_CMD:
        response = text_complete_last_message(
            history, stop_words_ids=stop_words_ids, gen_kwargs=gen_kwargs)
    else:
...
async def predict(
    model_inputs, history: List[List[str]], model_id: str, stop_words: List[str], gen_kwargs: Dict,
):
    global model, tokenizer
    ...
    print(model_inputs)
    inputs = tokenizer([model_inputs], return_tensors="pt").to(model.device)
    generation_kwargs = {
        **inputs,
        "streamer": streamer,
    }
    ...
    for _new_response in streamer:
        if len(_new_response) <= current_length:
            continue
        new_response = _new_response[:-
                                     current_length] if current_length else _new_response
...
def _get_args():
    ...
    parser.add_argument(
        ...
        "--checkpoint-path",
        type=str,
        default="/mnt/e/model_hub/Qwen2-7B-Instruct-GPTQ-Int4",
